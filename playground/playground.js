...
const hash = {
    ...
    grab: function (distanceCells, x, y) {
    ...
    render: function (distanceCells, x, y) {
...
function cubicNoiseSample(config, x, y) {
    ...
    for (let i = 0; i < 4; ++i) {
        ...
        xSamples[i] = interpolate(randomize(config.seed, x0, y), randomize(config.seed, x1, y), randomize(config.seed, x2, y), randomize(config.seed, x3, y), lerpX);
    }
    return interpolate(xSamples[0], xSamples[1], xSamples[2], xSamples[3], lerpY) * 0.5 + 0.25;
}
//function for generating a chunk
function tileGen(seed, startX, startY) {
    //initialise arrays
    ...
    for (let i = 0; i < columns; i++) {
        for (let j = 0; j < rows; j++) {
            ...
            //type assignment
            if (noiseOut > 179 && noiseOut < 255)
                sheet[i][j].type = "flowers";
            if (noiseOut > 79 && noiseOut < 180)
                sheet[i][j].type = "grass";
            if (noiseOut > 64 && noiseOut < 80)
                sheet[i][j].type = "tallGrass";
            if (noiseOut > 54 && noiseOut < 65)
                sheet[i][j].type = "bush";
            if (noiseOut > 0 && noiseOut < 55)
                sheet[i][j].type = "tree";
